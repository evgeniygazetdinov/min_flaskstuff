...
def test_create_vpn_config(mock_etcd):
    test_config = {"server": "test.vpn.com", "port": 1194, "protocol": "udp"}
    ...
    updated_config = {"server": "new.vpn.com", "port": 443, "protocol": "tcp"}
    response = client.put(
        f"/api/v1/config/{config_id}", json={"config_data": updated_config}
    )
    assert response.status_code == 200
